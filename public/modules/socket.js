...
export default class Socket {
    ...
    onSocketMessage(event) {
        ...
        if (message.type === "ru.mail.park.mechanics.requests.toUsers.PiratMoveMessage$Request") {
            ...
            content = JSON.parse(message.content);
            //  {"movement":"[{\"type\":\"PiratMove\",\"piratId\":0,\"targetCellIndex\":79,
            //\"status\":0,\"playerIngameId\":0}]","active":false}

            console.log("message:", content);
            this.MovementEvent.content = content;
            let movement = JSON.parse(content.movement);
            document.dispatchEvent(new CustomEvent("playermove",{
                detail: "Ходит игрок "+ movement[0].playerIngameId
            }));
